	...
	while (frames_left > 0)
	{
		...
		for (int frame = 0; frame < frame_count; frame++)
		{
			for (int channel = 0; channel < layout->channel_count; channel += 1)
			{
				...
				shared_in->buffer->data[channel][shared_in->write_pos] = *ptr;
			}
			shared_in->write_pos = (shared_in->write_pos + 1) % shared_in->buffer->num_frames;
		}
...
int AudioIn::init()
{
	...
	this->soundio = ((AudioOut *) this->graph->output.get())->soundio;
...
void AudioIn::next(sample **out, int num_frames)
{
