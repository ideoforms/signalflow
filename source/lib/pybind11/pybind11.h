        ...
        for (auto *pc = text; *pc != '\0'; ++pc)
        {
            ...
            else if (c == '%')
            {
                ...
                if (auto tinfo = detail::get_type_info(*t))
                {
                    ...
                    signature += th.attr("__module__").cast<std::string>() + "." + th.attr("__qualname__").cast<std::string>(); // Python 3.3+, but we backport it to earlier versions
                }
...
protected:
        ...
        void *get_buffer_data)
    {
        ...
        if (!type->ht_type.tp_as_buffer)
            pybind11_fail(
                "To be able to register buffer protocol support for the type '" + get_fully_qualified_tp_name(tinfo->type) + "' the associated class<>(..) invocation must "
...
public:
        ...
        : class_<Type>(scope, name, extra...), m_base(*this, scope)
    {
        ...
        attr("__setstate__") = cpp_function(
            [](detail::value_and_holder &v_h, Scalar arg) { detail::initimpl::setstate<Base>(v_h, static_cast<Type>(arg),
