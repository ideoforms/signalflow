...
void init_python_graph(py::module &m)
{
    ...
    py::class_<AudioGraph>(m, "AudioGraph")
        ...
        .def("start", &AudioGraph::start,
             R"pbdoc(Start the AudioGraph processing.)pbdoc")
        .def(
            "stop", [](AudioGraph &graph) { graph.stop(); },
            R"pbdoc(Stop the AudioGraph processing.)pbdoc")
        ...
        .def("render_to_buffer", &AudioGraph::render_to_buffer, "buffer"_a,
             R"pbdoc(Render the graph's output to the specified buffer, for the same number of frames as the buffer's length.)pbdoc")
        .def("render_to_new_buffer", &AudioGraph::render_to_new_buffer, "num_frames"_a,
