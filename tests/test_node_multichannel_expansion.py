...
from . import graph, SIGNALFLOW_UNIT_TEST_BUFFER_SIZE
...
def test_expansion_upmix():
    ...
    assert count_zero_crossings(a.output_buffer[0]) == math.ceil(440 * SIGNALFLOW_UNIT_TEST_BUFFER_SIZE / graph.sample_rate)
    assert count_zero_crossings(a.output_buffer[1]) == math.ceil(880 * SIGNALFLOW_UNIT_TEST_BUFFER_SIZE / graph.sample_rate)
    assert count_zero_crossings(a.output_buffer[2]) == math.ceil(1320 * SIGNALFLOW_UNIT_TEST_BUFFER_SIZE / graph.sample_rate)
